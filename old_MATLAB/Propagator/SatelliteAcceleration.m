function [ dY ] = SatelliteAcceleration( t, Y, mjd0, mu, Re, J2, ...
    ...
    rw_A_mat, rw_A_MPinv_mat, rw_I_mat, rw_maxAcc, ...
    rw_maxVel, controller_rw_Kp, controller_rw_Kd, ...
...
if enableMTQ
    ...
    mtq_b = [mtq_b; [b_earth_body;t]'];

    [m_mtq_body, t_pdref_sat_mtq] = MTQ_PD( qError_ECI, ...
        omegaError_Body, b_earth_body, sat_I_mat, ...
        controller_mtq_Kp, controller_mtq_Kd, mtq_maxDipoleMoment, ...
        w_sat_body, rw_A_mat, w_rw_w );
    
    mtq_m = [mtq_m; [m_mtq_body;t]'];
    mtq_t_cmd = [mtq_t_cmd; [t_pdref_sat_mtq;t]'];
    
    %m_mtq_body = MTQ_BDOT( b_earth_body, t, mtq_maxDipoleMoment );
    
    t_mtq_body = Magnetorquer_Torque( m_mtq_body, b_earth_body );
    
    mtq_t = [mtq_t; [t_mtq_body;t]'];
    
    control_torques = control_torques + t_mtq_body; %t_pdref_sat_mtq; %
...
if enableRW
    rw_commandedTorque = RW_NonlinPD( qError_ECI, omegaError_Body, ...
        sat_I_mat, w_sat_body, w_rw_w, rw_A_mat, rw_I_mat, ...
        rw_maxAcc, rw_maxVel, t_MTQ, ...
        controller_rw_Kp, controller_rw_Kd );
    
     [wdot_rw_w, t_rw] = RW_Torque( w_sat_body, rw_commandedTorque, w_rw_w, ...
...
wdot_sat = (sat_I_mat) \ ( ...
    -1 .* (S_crossProdMat(w_sat_body) * ((sat_I_mat + ...
    rw_A_mat * rw_I_mat * rw_A_MPinv_mat ) * w_sat_body ...
    + rw_A_mat * rw_I_mat * w_rw_w )) ...
    + disturbance_torques ...
    + control_torques ...
    );
...
dY = [ rdot; vdot; qdot; wdot_sat; wdot_rw_w; zeros(4,1) ];
